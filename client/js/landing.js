...
(function() {
  ...
  function stableHash(str) {
    let h = 0;
    for (let i = 0; i < str.length; i++) { const c = str.charCodeAt(i); h = ((h << 5) - h + c) | 0; }
    return 'h' + (h >>> 0).toString(16) + '_' + str.length;
  }

  function deviceSignals() {
    const sa = typeof screen !== 'undefined' ? screen : {};
    let plat = '';
    try { plat = (navigator.userAgentData && navigator.userAgentData.platform) || navigator.platform || ''; } catch (e) { try { plat = navigator.platform || ''; } catch (e2) {} }
    return {
      cores: (typeof navigator.hardwareConcurrency === 'number') ? navigator.hardwareConcurrency : 0,
      mem: (typeof navigator.deviceMemory === 'number') ? navigator.deviceMemory : 0,
      platform: plat || '',
  ...
  // Stable hardware-only fingerprint: identical across browsers/profiles on the
  // same device. Used for device-level bans that survive "change browser / clear
  // storage" bypass attempts.
  function getHardwareFp() {
    const s = deviceSignals();
    return stableHash([s.cores, s.mem, s.platform, s.touch, s.w, s.h, s.cd, s.tz, s.lang].join('|'));
  }
  window.getHardwareFp = getHardwareFp;
  window.getDeviceInfoJson = function () {
    try {
      return JSON.stringify(Object.assign({}, deviceSignals(), { ua: navigator.userAgent, fp2: getHardwareFp() }));
    } catch (e) { return ''; }
  };

  async function getFingerprint() {
    try {
      const cached = localStorage.getItem('chat_fingerprint');
    ...
    try {
      if (typeof window.FingerprintJS !== 'undefined') {
        ...
        try { localStorage.setItem('chat_fingerprint', vid); } catch (e) {}
        return vid;
      }
    } catch (e) {}
    try {
      const dyn = await import('https://openfpcdn.io/fingerprintjs/v4');
      const FP = dyn && dyn.default ? dyn.default : null;
    ...
    // Deterministic fallback (not random): survives page reloads and storage clear.
    const s = deviceSignals();
    const sig = JSON.stringify(s) + '|' + navigator.userAgent;
    const fallbackFp = 'fp_' + stableHash(sig);
    try { localStorage.setItem('chat_fingerprint', fallbackFp); } catch (e) {}
    return fallbackFp;
  }

  async function deviceAuthExtra() {
    try {
      return { fp2: getHardwareFp(), deviceInfo: window.getDeviceInfoJson ? window.getDeviceInfoJson() : '' };
    } catch (e) { return {}; }
  }
  ...
  function showAuthForm(formName) {
    ...
    [memberForm, guestForm, registerForm].forEach(f => {
      if (f) {
        ...
        // Hard-hide inline so forms never stack even if CSS is slow/blocked
  ...
  const handleMemberLogin = async () => {
    ...
    try {
      ...
      const extra = await deviceAuthExtra();
      const clientSessionId = window.createNewClientSessionId();
      const data = { username, password, isHidden, fp: fingerprint, clientSessionId, ...extra };
  ...
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      const extra = await deviceAuthExtra();
      const clientSessionId = window.createNewClientSessionId();
      const data = { nickname, fp: fingerprint, clientSessionId, ...extra };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const extra = await deviceAuthExtra();
      const clientSessionId = window.createNewClientSessionId();
      const data = { username, password, fp: fingerprint, clientSessionId, ...extra };
    ...
    try {
      ...
      if (!window.__mainScriptLoadingPromise && typeof window.completeChatLogin !== 'function') {
        window.__mainScriptLoadingPromise = new Promise((resolve, reject) => {
          ...
          mainScript.src = '/dist/chat-main.bmt3r5hbb.js';
