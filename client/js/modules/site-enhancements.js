...
export function prettifySystemMessage(raw) {
  if (!raw) return raw;
  var t = String(raw);
  if (t.indexOf('هذا المستخدم قد دخل') !== -1) {
    return t.replace(/هذا المستخدم قد دخل.*/g, '✨ هذا المستخدم نوّر المكـان');
  }
  if (t.indexOf('إنتقل إلى غرفة') !== -1 || t.indexOf('انتقل إلى غرفة') !== -1) {
    return t.replace(/هذا المستخدم (إنتقل|انتقل) إلى غرفة\s*$/, 'هذا المستخدم إنتقل إلى غرفة أخرى');
  }
  return t;
}
